...
class Attention(nn.Module):
    ...
    def __init__(self, embed_size, num_heads, query_and_keys_head_size, values_head_size, dropout_prob) -> None:
        super().__init__()


        self.query_projection = nn.Linear(embed_size, num_heads * query_and_keys_head_size, bias = False)
        self.key_projection = nn.Linear(embed_size, num_heads * query_and_keys_head_size, bias = False)
        self.value_projection = nn.Linear(embed_size, num_heads * values_head_size, bias = False)
        self.output_projection = nn.Linear(num_heads * values_head_size, embed_size)
        self.dropout = nn.Dropout(dropout_prob)

        self.query_and_keys_head_size = torch.tensor(query_and_keys_head_size)
    ...
    def forward(self, qs, ks, vs, mask = None):
        ...
        qs = self.query_projection(qs).view(batch_size, qs.shape[1], self.num_heads, self.query_and_keys_head_size)
        ks = self.key_projection(ks).view(batch_size, ks.shape[1], self.num_heads, self.query_and_keys_head_size)
        vs = self.value_projection(vs).view(batch_size, vs.shape[1], self.num_heads, self.values_head_size)

        attn_product = qs.permute(0,2,1,3) @ ks.permute(0,2,3,1) / torch.sqrt(self.query_and_keys_head_size)
        ...
        output = self.output_projection(res.transpose(1,2).reshape(batch_size, -1, self.values_head_size * self.num_heads))
